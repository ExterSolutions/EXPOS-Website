...
function CartList({
    ...
    const handleEdit = () => {
        ...
        if (cData?.productType === "special_pizza") {
            if (location.pathname === `/specialoffer/${cData?.productCode}/${cData?.id}`) {
                setShowSidebar(false);
            } else {
                handleRedirectToEdit(`/specialoffer/${cData?.productCode}/${cData?.id}`);
                setShowSidebar(false);
            }
        }
                ...
                <div className="col-12">
                    <div className="row align-items-center">
                        ...
                        <div className="col-md-9 col-8 text-start productTitle">
                            <span className="fw-bold">{cData?.productName} x {cData?.quantity}</span>
                        </div>
                        {/* Price */}
                        <div className="col-md-3 col-4 text-end">
                            <span className="fw-bold" style={{ fontSize: "1rem" }}>${cData?.amount}</span>
                        </div>
                    </div>
                </div>
            </div>
