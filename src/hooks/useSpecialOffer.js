    ...
    useEffect(() => {
        ...
        const fetchAll = async () => {
            try {
                ...
                const settingsObj = {
                    premiumToppingsCount: settingsData.find((s) => s.shortCode === "non-regular-toppings-count")?.settingValue ?? 1,
                    premiumTopppingLabel: settingsData.find((s) => s.shortCode === "non-regular-toppings")?.settingValue ?? "Premium Toppings",
                    regularToppingLabel: settingsData.find((s) => s.shortCode === "regular-toppings")?.settingValue ?? "Regular Toppings",
                };
