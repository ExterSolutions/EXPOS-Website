...
const EditSpecialOfferPage = () => {
    ...
    useEffect(() => {
        ...
        setTotalPrice(total);
    }, [offerData, selectedSize, selectedSide, selectedDrink, selectedDips, pizzaSelections]);

    const fetchOfferDetailsWithSettings = async (cartItem) => {
        try {
            const [specialOfferResponse, settingsResponse, toppingsResponse, dipsResponse] = await Promise.all([
            ...
            if (specialOfferData.pizza_prices?.length > 0) {
                allPrices = specialOfferData.pizza_prices.filter((p) => Number(p.price) > Number(0));
            ...
            if (cartItem) {
                // Set Size
                const sizeFromCart = specialOfferData.pizza_prices.find(
    ...
    const handleUpdateCart = (e) => {
        ...
        const hasMissingSignature = pizzaSelections.some(
            (pizza) => !pizza.signaturePizzaCode || pizza.signaturePizzaCode.trim() === ""
        );

        if (hasMissingSignature) {
            toast.error("Please select a signature pizza for all pizzas before updating cart.");
            return;
        }
        ...
        const itemIndex = ct.product.findIndex(item => item.id === cartItemToEdit.id);
    ...
    if (!offerData) {
        toast.error("Unable to load offer details. Could you please check out our menu?", {
            position: "top-center",
            autoClose: 3000,
            onClose: () => {
                navigate("/menu");
            ...
            <div className="container py-4">
                ...
                <div className="row">
                    ...
                    <div className="col-lg-6 col-md-7 col-12">
                        ...
                        <p className="text-muted">Select any and customize as you like from our special pizzas.</p>
