...
const EditCreatePizza = () => {
  ...
  const [price, setPrice] = useState(
    cart?.product?.find((el) => el?.id == uid)?.amount || 0
  );
  //
  ...
  // setting initial data for edit
  useEffect(() => {
    if (uid) {
      const cProduct = cart?.product?.find((el) => el?.id == uid);
      if (cProduct) {
        let freeToppingsArr =
          cProduct?.config?.pizza[0]?.toppings?.freeToppings?.length > 0
            ? cProduct?.config?.pizza[0]?.toppings?.freeToppings?.map((el) => {
                return {
                  code: el?.toppingsCode,
                  name: el?.toppingsName,
                  price: el?.toppingsPrice,
                  type: "free",
        ...
        let oneToppingsArr =
          cProduct?.config?.pizza[0]?.toppings?.countAsOneToppings?.length > 0
            ? cProduct?.config?.pizza[0]?.toppings?.countAsOneToppings?.map(
                (el) => {
                  return {
                    code: el?.toppingsCode,
                    name: el?.toppingsName,
        ...
        let twoToppingsArr =
          cProduct?.config?.pizza[0]?.toppings?.countAsTwoToppings?.length > 0
            ? cProduct?.config?.pizza[0]?.toppings?.countAsTwoToppings?.map(
                (el) => {
                  return {
                    code: el?.toppingsCode,
                    name: el?.toppingsName,
        ...
        const indianStyleToppings =
          freeToppingsArr?.length > 0
            ? freeToppingsArr?.map((el) => {
                return {
                  code: el?.code,
                  name: el?.name,
                  price: el?.price,
        ...
        const oneToppings =
          oneToppingsArr?.length > 0
            ? oneToppingsArr?.map((el) => {
                return {
                  code: el?.code,
                  name: el?.name,
                  price: el?.price,
        ...
        const twoToppings =
          twoToppingsArr?.length > 0
            ? twoToppingsArr?.map((el) => {
                return {
                  code: el?.code,
                  name: el?.name,
                  price: el?.price,
        ...
        const drinksArray =
          cProduct?.config?.drinks?.length > 0
            ? cProduct?.config?.drinks?.map((el) => {
                return {
                  code: el?.drinksCode,
                  name: el?.drinksName,
                  quantity: el?.quantity,
                  price: el?.drinksPrice,
                  totalPrice: el?.totalPrice,
                };
              })
            : [];
        const dipsArray =
          cProduct?.config?.dips?.length > 0
            ? cProduct?.config?.dips?.map((el) => {
                return {
                  code: el?.dipsCode,
                  name: el?.dipsName,
                  quantity: el?.quantity,
        ...
        const sidesArray =
          cProduct?.config?.sides?.length > 0
            ? cProduct?.config?.sides?.map((el) => {
                return {
        ...
        if (cProduct?.config?.pizza[0]?.specialBases?.specialbaseCode) {
          setSpecialBases(
            cProduct?.config?.pizza[0]?.specialBases?.specialbaseCode
          );
        }
        setToppingsFree(freeToppingsArr);
        setToppingsOne(oneToppingsArr);
        setToppingsTwo(twoToppingsArr);
  ...
  useEffect(() => {
    if (
      ...
      let price = 0;
      price = +price * +pizzaQuantity;
      if (pizzaSizeArr?.length > 0) {
        let sizeObject = Ingredients?.sizesAndPrices?.find(
          (el) => el?.size === size
        );
        let new_price = sizeObject?.price * +pizzaQuantity;
            ...
            <section className="primary-background-color special-offers-sec new-block ">
              ...
              <div className="container">
                <div className="mainContainer primary-text-color">
                  {/* left side */}
                  <div className=" p-3">
                    <p className="fs-1 fw-bold">CREATE YOUR OWN PIZZA</p>
                    ...
                    >
                      <div
                        className={`p-3 card-background-color card-text-color ${
                          isFixed ? "fixed" : ""
                        }`}
                        style={{
                          ...
                          <h2 className="accordion-header" id="headingOne">
                            <button
                              className={`fw-bold fs-6 accordion-button ${
                                activeAccordion === "size" ? "" : "collapsed"
                              }`}
                              type="button"
                      ...
                      <div className="accordion" id="accordionExample4">
                        <div className="accordion-item">
                          <h2 className="accordion-header" id="headingFour">
                            <button
                              className={`fw-bold fs-6 accordion-button ${
                                activeAccordion === "dough" ? "" : "collapsed"
                              }`}
                              type="button"
                      ...
                      <div className="accordion" id="accordionExample4">
                        <div className="accordion-item">
                          <h2 className="accordion-header" id="headingFour">
                            <button
                              className={`fw-bold fs-6 accordion-button ${
                                activeAccordion === "cheese" ? "" : "collapsed"
                              }`}
                              type="button"
                      ...
                      <div className="accordion" id="accordionExample6">
                        <div className="accordion-item">
                          <h2 className="accordion-header" id="headingSix">
                            <button
                              className={`fw-bold fs-6 accordion-button ${
                                activeAccordion === "spicy" ? "" : "collapsed"
                              }`}
                              type="button"
                      ...
                      <div className="accordion" id="accordionExample7">
                        <div className="accordion-item">
                          <h2 className="accordion-header" id="headingSeven">
                            <button
                              className={`fw-bold fs-6 accordion-button ${
                                activeAccordion === "sauce" ? "" : "collapsed"
                              }`}
                              type="button"
                      ...
                      <div className="accordion" id="accordionExample8">
                        <div className="accordion-item">
                          <h2 className="accordion-header" id="headingEight">
                            <button
                              className={`fw-bold fs-6 accordion-button ${
                                activeAccordion === "cook" ? "" : "collapsed"
                              }`}
                              type="button"
                          ...
                          >
                            <div className="accordion-body primary-background-color">
                              <div className="pb-2 mb-2 d-flex justify-content-between row">
                                <div
                                  className={`cursor-pointer py-2 col-4 lh-sm text-center card-text-color ${
                      ...
                      <div className="accordion" id="accordionExample10">
                        <div className="accordion-item">
                          <h2 className="accordion-header" id="headingTen">
                            <button
                              className={`fw-bold fs-6 accordion-button ${
                                activeAccordion === "sides" ? "" : "collapsed"
                              }`}
                              type="button"
                      ...
                      <div className="accordion" id="accordionExample10">
                        <div className="accordion-item">
                          <h2 className="accordion-header" id="headingTen">
                            <button
                              className={`fw-bold fs-6 accordion-button ${
                                activeAccordion === "drinks" ? "" : "collapsed"
                              }`}
                              type="button"
                      ...
                      <div className="accordion" id="accordionExample11">
                        <div className="accordion-item">
                          <h2 className="accordion-header" id="headingEleven">
                            <button
                              className={`fw-bold fs-6 accordion-button ${
                                activeAccordion === "dips" ? "" : "collapsed"
                              }`}
                              type="button"
                  ...
                  >
                    <div
                      className={`p-3 right-side-internal-div card-background-color card-text-color ${
                        isFixed ? "fixed" : ""
                      }`}
                      style={{
                        transform: isTranslate
                      ...
                      <div className="px-3 row">
                        ...
                        <div className="col-lg-6 p-4">
                          <div className="d-flex flex-column py-4">
                            ...
                            <div className="d-none gap-3 justify-content-center  justify-content-lg-start align-items-center mt-4">
                              <button
                                disabled={pizzaQuantity <= 1}
                                onClick={() =>
                                  setPizzaQuantity((prev) => prev - 1)
                              ...
                              <p className=" fs-4 fw-bold">{pizzaQuantity}</p>
                              <button
                                className="btn btn-light rounded-circle"
                                onClick={() =>
                                  setPizzaQuantity((prev) => prev + 1)
                                ...
                                <div className="col-lg-6">
                                  {size && (
                                    <p className=" fs-6 mt-2 mt-lg-0">
                                      <GoDotFill /> Size: {size}({pizzaQuantity}
                                      ) ($
                                      {+pizzaSizeArr?.find(
                                        (el) => el?.size === size
                                      )?.price * +pizzaQuantity}
...
export default EditCreatePizza;
